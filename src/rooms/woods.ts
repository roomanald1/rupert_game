...
export class Woods implements Room {
    ...
    async use_item(engine: GameEngine, item: Items): Promise<void> {
        if (item == Items.sword) {
            engine.clear_screen();
            engine.write_line("You stumble forwards. And snap a twig under your feet. Now, the howling sound is much closer and you are surrounded by red eyes.")
            engine.write_line("Its a pack of hungry wolves - You brandish your sword - and defeat the wolves");
    ...
    async visit(engine: GameEngine, from: Rooms): Promise<void> {
        ...
        if (engine.event_occurred(Events.defeat_wolves)) {
            await engine.prompt_options("What would you like to do?",
                [
                    { optionDescription: "Head back to the garden", action: async () => engine.move_to_room(Rooms.garden) },
                ]
            )
        } else {
            await engine.prompt_options("What would you like to do?",
